package com.shenfeng.yxw.elasticsearch.service.impl;
...
import com.shenfeng.yxw.elasticsearch.dal.ShopModelMapper;
...
import com.shenfeng.yxw.elasticsearch.service.SellerService;
...
public class ShopServiceImpl implements ShopService {
    ...
    public Map<String, Object> searchES(BigDecimal longitude, BigDecimal latitude, String keyword, Integer orderby, Integer categoryId, String tags) {
        ...
        for (SearchHit searchHit : hits) {
            list.add(new Integer(searchHit.getSourceAsMap().get("id").toString()));
        }
        // 根据id获取实体类
        List<ShopModel> shopModelList = list.stream().map(id -> {
            return get(id);
    ...
    @Override
    public Map<String, Object> searchES2(BigDecimal longitude, BigDecimal latitude, String keyword, Integer orderby, Integer categoryId, String tags) {
        Map result = new HashMap();
        Request searchRequest = new Request("GET", "/shop/_search");
        String json = "{\n" +
                "  \"_source\": \"*\",\n" +
                "  \"script_fields\": {\n" +
                "    \"distance\": {\n" +
        ...
        searchRequest.setJsonEntity(json);
        Response searchResponse = null;
        String toString = null;
        try {
            searchResponse = highLevelClient.getLowLevelClient().performRequest(searchRequest);
            toString = EntityUtils.toString(searchResponse.getEntity());
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        for (int i = 0; i < array.size(); i++) {
            ...
            String s = object.getJSONObject("fields").getJSONArray("distance").get(0).toString();
            BigDecimal distance = new BigDecimal(s);
            log.info("distance:{}", distance);
            ShopModel shopModel = get(id);
            shopModel.setDistance(
                    distance.multiply(new BigDecimal
                            (1000).setScale(0, BigDecimal.ROUND_CEILING)).intValue());
            list.add(shopModel);
        }

        result.put("shop", list);
        return result;
    }


    @Override
    public Map<String, Object> searchES3(BigDecimal longitude, BigDecimal latitude, String keyword, Integer orderby, Integer categoryId, String tags) {
        Map result = new HashMap();
        Request searchRequest = new Request("GET", "/shop/_search");
        //构建请求
        ...
        log.info("params:{}", reqJson);
        searchRequest.setJsonEntity(reqJson);
        Response searchResponse = null;
        String toString = null;
        try {
            searchResponse = highLevelClient.getLowLevelClient().performRequest(searchRequest);
    ...
    @Override
    public Map<String, Object> searchES4(BigDecimal longitude, BigDecimal latitude, String keyword, Integer orderby, Integer categoryId, String tags) {
        Map result = new HashMap();
    ...
    //构造分词函数识别器
    private Map<String, Object> analyzeCategoryKeyword(String keyword) {
    ...
    // 构造分词函数识别器
    private Map<Integer, List<String>> categoryWorkMap = new ConcurrentHashMap();
